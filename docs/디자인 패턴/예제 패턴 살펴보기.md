...
만약 기존의 템플릿을 벗어나는단계가 발생한다면 Hook(후크)를 활용하여 필요한 서브클래스에서만 활용하면 됩니다. 후크는 필요하면 구현하고 아니면 구현하지 않아도 되는(= 기본값을 사용해도 상관없는) 메서드를 말한다. 예를 들면 MongoDB 등의 NoSQL은 연결이 종료된 이후에 별도의 로직이 필요하다 가정하면, 아래와 같이 슈퍼 클래스에 후크를 추가하고 NoSQL들만 추가로 구현하면 방식이다.
